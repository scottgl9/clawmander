...
import ApprovalEmojiActions from './ApprovalEmojiActions';
...
export default function ChatMessage({ message, onSpeak, onRetry, onResolveApproval }) {
          ...
          {message.approval ? (
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="text-xs font-semibold text-yellow-300 mb-1">Exec Approval Required</div>
                <div className="font-mono text-xs bg-gray-950/80 border border-gray-700 rounded px-2 py-1 truncate" title={message.approval.commandText || message.approval.command}>
                  {message.approval.command || message.approval.commandText || 'Command approval requested'}
                </div>
                <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-gray-500">
                  {message.approval.host && <span>Host: {message.approval.host}</span>}
                  {message.approval.nodeId && <span>Node: {message.approval.nodeId}</span>}
                  {message.approval.cwd && <span className="truncate max-w-full">CWD: {message.approval.cwd}</span>}
                </div>
                {message.approval.warningText && (
                  <div className="mt-1 text-xs text-yellow-200/80 whitespace-pre-wrap">{message.approval.warningText}</div>
                )}
              </div>
              <ApprovalEmojiActions approval={message.approval} onResolve={onResolveApproval} />
            </div>
          ) : (
