...
export default function ChatPage({ onConnectionChange }) {
  ...
  const activeModel = useMemo(() => {
    ...
    const alias = activeSessionObj.modelAlias || activeSessionObj.alias;
    if (alias) return alias;
    const meta = models?.find((m) => m.id === activeSessionObj.model);
    return meta?.name || activeSessionObj.model || null;
  }, [activeSessionObj, models]);
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className={`${!mobileSidebarOpen ? 'flex' : 'hidden'} md:flex flex-col flex-1 min-w-0 overflow-hidden`}>
        ...
        <div className="hidden md:flex items-center">
          ...
          {activeModel && (
            <span className="flex-shrink-0 mr-3 text-xs text-gray-500 truncate max-w-[180px]" title={activeModel}>
              {activeModel}
            </span>
          )}
          <button
