...
All services share one Tailscale hostname (`homelab.ts.net`) with nginx routing by path. TLS is terminated at nginx using Tailscale-managed certificates.
...
```bash
sudo tailscale cert \
  --cert-file /etc/ssl/tailscale/homelab.ts.net.crt \
  --key-file  /etc/ssl/tailscale/homelab.ts.net.key \
  homelab.ts.net
```
...
This means:
- **Browser visiting `https://homelab.ts.net/`** → Clawmander frontend
- **Android app connecting `wss://homelab.ts.net`** → OpenClaw Gateway
...
```bash
# Clawmander frontend
curl -s https://homelab.ts.net/ | grep -o '<title>.*</title>'

# OpenClaw Control UI
curl -sI https://homelab.ts.net/openclaw/

# Matrix
curl -s https://homelab.ts.net/_matrix/client/versions | head -1
